...
import pymysql, json, os, logging
from datetime import datetime, timedelta

logger = logging.getLogger('xiyi_fill')
...
logger.info(f"  ds_source_connection: {len(conns)}条 ✓")
...
logger.info(f"  ds_source_heartbeat: {len(conn_ids)}条 ✓")
...
logger.info(f"  ds_table_metadata: {len(tables_data)}条 ✓")
...
logger.info(f"  ds_column_metadata: {len(cols)}条 ✓")
...
if first_col_id:
    ...
    logger.info(f"  ds_mapping_rule: {len(rules)}条 ✓")
else:
    logger.info("  ds_mapping_rule: 跳过(无column元数据) ✗")
...
logger.info(f"  dg_data_standard: {len(standards)}条 ✓")
...
logger.info(f"  dg_derived_indicator: {len(derived)}条 ✓")
...
for i in range(1, 13):
    ...
    for ind_code, base_val in [('FPY_RATE', 97.5), ('MAG_ABNORM_RATE', 3.2), ('COQ_RATE', 1.5),
        ...
        snapshot_count += 1
logger.info(f"  dg_indicator_snapshot: {snapshot_count}条 ✓")
...
logger.info(f"  dg_quality_check_rule: {len(qc_rules)}条 ✓")
...
logger.info(f"  vw_alert_record: {len(alerts)}条 ✓")
...
logger.info(f"  ap_analysis_model: {len(models)}条 ✓")
...
logger.info(f"  ap_scene_ds_binding: 7场景 ✓")
...
logger.info(f"  ap_scene_report_tpl: {len(templates)}条 ✓")
...
logger.info(f"  sys_config: {len(sys_cfgs)}条 ✓")
